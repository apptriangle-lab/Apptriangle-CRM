...
import { CRM_TASKS_LIST_HPAD, CRM_TASKS_TITLE_PL } from "@/components/tasks/crmTasksListStyles";
...
function PmsHubTasksListSkeletonRow({ index }: { index: number }) {
  const titleWidth = ["w-[72%]", "w-[58%]", "w-[84%]", "w-[65%]", "w-[76%]"][index % 5];

  return (
    <div className={cn(PMS_HUB_TASKS_COL_GRID, "border-b border-slate-100 py-2.5")}>
...
export function PmsHubTasksListSkeleton() {
  return (
        ...
        <div className={PMS_HUB_TASKS_TABLE_MIN_W}>
          ...
          <div className={CRM_TASKS_LIST_HPAD}>
            {Array.from({ length: SKELETON_ROW_COUNT }, (_, i) => (
              <PmsHubTasksListSkeletonRow key={i} index={i} />
            ))}
          </div>
        </div>
      </div>
