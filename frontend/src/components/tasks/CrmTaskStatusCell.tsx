...
import { crmTaskStatusDotClass, crmTaskStatusPillClass } from "@/components/tasks/crmTaskStatusStyles";
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm font-medium text-slate-900">{formatStatusLabel(statusKey)}</p>
      </div>
...
export function CrmTaskStatusCell({
  status,
  statuses,
  canChange,
  onStatusChange,
      ...
      <DropdownMenu open={open} onOpenChange={setOpen}>
        <DropdownMenuTrigger asChild>
          <button
            ...
            aria-label={`Change status, currently ${formatStatusLabel(status)}`}
          >
            {pill}
          </button>
        </DropdownMenuTrigger>
...
export function canChangeCrmTaskStatus(
  task: { status: string; assignByUserId: string; assignToUserId: string },
  userId: string | undefined,
  tasksScopeAdmin: boolean,
): boolean {
  if (task.status === "completed" && !tasksScopeAdmin) return false;
  return tasksScopeAdmin || task.assignByUserId === userId || task.assignToUserId === userId;
}
